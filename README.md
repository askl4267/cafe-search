...
Cloudflare Pages ships that worker as a Pages Function defined in `functions/worker.js`, which re-exports the generated `worker.js` copy and declares the `route: "/*"`/`runtime: "edge"` config that Pages needs to wire it in. The `npm run build:cloudflare` step (via `opennextjs-cloudflare`) now copies `.open-next/worker.js` into `functions/worker.bundle.js` so the Functions compiler can bundle it—this copy happens automatically via the new `postbuild:cloudflare` script. Once both the copy and `functions/worker.js` are in place, the preview domain (`*.pages.dev`) will start serving your SSR app instead of static 404s.
