...
Cloudflare Pages ships that worker as a Pages Function defined in `functions/worker.mjs`, which re-exports the generated `worker.js` and declares the `route: "/*"`/`runtime: "edge"` config that Pages needs to wire it in. Once `npm run build:cloudflare` produces `.open-next/worker.js` and this function exists, the preview domain (`*.pages.dev`) will start serving your SSR app instead of static 404s.
